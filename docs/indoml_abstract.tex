...
We trained Standard PPO for \textbf{1,000,000 (1.0M) timesteps} and Maskable PPO for \textbf{2,000,000 (2.0M) timesteps} against 3 rule-compliant heuristic opponents:
\begin{enumerate}[leftmargin=*,noitemsep,topsep=2pt]
    \item \textbf{Standard PPO (Unmasked, 1.0M Steps):} Relies on penalty shaping ($r_{\text{invalid}} = -2.0$) to learn valid moves.
    \item \textbf{Maskable PPO (Masked, 2.0M Steps):} Employs invalid action masking with linear learning rate decay ($3 \times 10^{-4} \to 5 \times 10^{-5}$) and tuned entropy coefficient ($\text{ent\_coef}=0.005$) to restrict policy distribution $\pi(a|s)$ exclusively to legal actions.
\end{enumerate}

\begin{table}[h]
\centering
\caption{Head-to-head benchmark performance over 1,000 matches (Standard PPO: 1.0M timesteps, Maskable PPO: 2.0M timesteps).}
\label{tab:results}
\small
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{Standard PPO (1.0M)} & \textbf{Maskable PPO (2.0M)} \\
\midrule
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\linewidth]{../evaluation/kori_khel/plots/ppo_vs_maskable_comparison.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\linewidth]{../evaluation/kori_khel/plots/training_curves_comparison.png}
